...
def extract_course_to_file(ci):
    ...
    # Ye headers security bypass ke liye sabse zaroori hain
    h = {
        'jwt': MY_JWT,
        'User-Agent': 'okhttp/4.9.1',
    ...
    try:
        with open(filename, "w", encoding="utf-8") as f:
            ...
            # Agar abhi bhi 403 aaye, toh message display karega
            if r4_resp.status_code == 403:
                return "Error: 403 Forbidden. Server is blocking the request. Try a fresh Token or use a VPN/Proxy."

            dr3_raw = decrypt_stream(r4_resp.json().get("response"))
            if not dr3_raw: return "Error: Decrypt Failed. Token expire ho gaya hai."
            
            dr3 = json.loads(dr3_raw)
            l1_data = dr3.get("data", {})
            items = l1_data.get("list", []) if isinstance(l1_data, dict) else l1_data

            if not items: return "Error: No content found."
            ...
            for item in items:
                ...
                r5 = session.post('https://online.utkarsh.com/web/Course/tiles_data', headers=h, data={'tile_input': encrypt_stream(json.dumps(d5))})
                dr4 = json.loads(decrypt_stream(r5.json().get("response")))
                ...
                for sub in sub_list:
                    ...
                    r6 = session.post('https://online.utkarsh.com/web/Course/get_layer_two_data', headers=h, data={'layer_two_input_data': b64encode(json.dumps(d7).encode()).decode()})
                    dr5 = json.loads(decrypt_stream(r6.json().get("response")))
                    ...
                    for top in top_list:
                        ...
                        r7 = session.post('https://online.utkarsh.com/web/Course/get_layer_two_data', headers=h, data={'layer_two_input_data': b64encode(json.dumps(d9).encode()).decode()})
                        dr6 = json.loads(decrypt_stream(r7.json().get("response")))
                        ...
                        for v in vids:
                            ...
                            if jti:
                                try:
                                    ...
                                    if url: f.write(f"{title}: {url.split('?Expires=')[0]}\n")
                                except: continue
        return filename

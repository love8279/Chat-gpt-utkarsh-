...
def decrypt_stream(enc):
    try:
        if not enc or enc == "": return None
        enc = b64decode(enc)
        # Standard Utkarsh Keys
...
def post_request(path, data, key, iv):
    # Meta distributor request logic
    ...
    try:
        with open(filename, "w", encoding="utf-8") as f:
            ...
            if r4_resp.status_code != 200:
                return f"Error: Server Status {r4_resp.status_code}"

            dr3_raw = decrypt_stream(r4_resp.json().get("response"))
            if not dr3_raw: return "Error: Layer 1 Decrypt Failed. Token may be expired."
            
            dr3 = json.loads(dr3_raw)
            l1_data = dr3.get("data", [])
            items = l1_data.get("list", []) if isinstance(l1_data, dict) else l1_data

            if not items: return "Error: No content found in this Batch ID."
